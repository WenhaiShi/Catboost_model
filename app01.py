...
if selected_features is None:
    selected_features = ['Age', 'LOS', 'Weight', 'SOFA', 'RR', 'SPO2', 'PH', 'PO2', 'PCO2', 'BE', 
                        'WBC', 'Lymphocyte_count', 'MCHC', 'Calcium', 'INR', 'Chloride', 'Glucose', 
                        'Output_urine_avg', 'Beta_bloker', 'Furosemide']
...
feature_ranges = {
    ...
    'BE': (-20, 20, 'mEq/L'),
    'WBC': (0, 50, '×10⁹/L'),
    'Lymphocyte_count': (0, 10, '×10⁹/L'),
    'MCHC': (20, 40, 'g/dL'),
    'Calcium': (0, 20, 'mg/dL'),
    'INR': (0, 10, ''),
    'Chloride': (80, 130, 'mmol/L'),
    'Glucose': (0, 1000, 'mg/dL'),
    'Output_urine_avg': (0, 5000, 'mL')
}
    ...
    with st.form("patient_form"):
        ...
        with input_col1:
            for i, feature in enumerate(selected_features[:10]):
                if feature in ['Beta_bloker', 'Furosemide']:
        ...
        if submitted:
            ...
            if missing_features:
                ...
                # 转换checkbox值
                for feature in ['Beta_bloker', 'Furosemide']:
                    if feature in input_values:
                        input_values[feature] = 1 if input_values[feature] else 0
...
- **Vital Signs & Blood Gas**: {', '.join([f for f in selected_features if f in ['RR', 'SPO2', 'PH', 'PO2', 'PCO2', 'BE']])}
- **Laboratory Values**: {', '.join([f for f in selected_features if f in ['WBC', 'Lymphocyte_count', 'MCHC', 'Calcium', 'INR', 'Chloride', 'Glucose']])}
- **Medications & Renal Function**: {', '.join([f for f in selected_features if f in ['Beta_bloker', 'Furosemide', 'Output_urine_avg']])}
""")
